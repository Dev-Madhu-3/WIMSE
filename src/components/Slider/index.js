...
const ImageSlider = () => {
  return (
    <Swiper
      ...
      loop={true}
      speed={800}
      className="w-full h-[95vh] min-h-[350px] bg-[var(--primary-color)] sm:h-[25vh] sm:min-h-[25vh]"
    >
      {slides.map((each, idx) => (
        <SwiperSlide key={idx}>
          <div
            className="flex flex-col justify-center items-center h-full text-black text-center p-5 bg-cover"
            style={{ backgroundImage: `url(${each.image})` }}
          >
            <h2
              className="m-0 opacity-0 animate-fade-in delay-[500ms] duration-1000 text-[aliceblue] bg-[rgba(36,36,36,0.276)]"
            >
              {each.heading || '.'}
            </h2>
            <p
              className="opacity-0 animate-slide-up delay-[500ms] duration-1000 text-[aliceblue] bg-[rgba(36,36,36,0.276)]"
            >
              {each.text}
            </p>
          </div>
